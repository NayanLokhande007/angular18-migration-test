...
@Component({
  selector: 'app-ad-approver-profile',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './ad-approver-profile.component.html',
  styleUrl: './ad-approver-profile.component.css'
})
export class AdApproverProfileComponent {
  data: any;
  options: any;
  approverDetails: any;
  ...
  graphvalues: any[]=[];
   ...
   constructor(private globalservice: GlobalServiceService, private router: Router, private fb: FormBuilder) {
    ...
    this.getPendingQuestionsOfApproverForAdmin();
   }
...
gotoEmplist(){
  ...
  getPendingQuestionsOfApproverForAdmin(){
  ...
  RenderBarChart(chartid: string, charttype: any) {
    ...
    this.globalservice.getMonthWiseApprovedVaritionsCountOfApprover(this.emailid, this.currentYear).subscribe(result => {
      const responsedata = JSON.parse(JSON.stringify(result));
      if (responsedata.status == 200) {
