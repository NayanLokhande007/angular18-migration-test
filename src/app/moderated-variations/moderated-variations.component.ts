...
@Component({
  selector: 'app-moderated-variations',
  standalone: true,
  imports: [CommonModule,ReactiveFormsModule],
  templateUrl: './moderated-variations.component.html',
  styleUrl: './moderated-variations.component.css'
})
export class ModeratedVariationsComponent { 
    approvedQue: any;
    queForm!: FormGroup;
    ...
    ngOnInit(): void {
      ...
      this.getModeratedQuestionsOfProgrammer();
    }
    ...
    getApprovedByAssesorQuestions() {
      this.globalservice.getApprovedByAssesorQuestions(this.emailid).subscribe(result => {
    ...
    getModeratedQuestionsOfProgrammer(){
      this.globalservice.getModeratedQuestionsOfProgrammer(this.emailid).subscribe(result => {
        const responsedata = JSON.parse(JSON.stringify(result));
        if (responsedata.status == 200) {
    ...
    applyFilter() {
      this.filteredQue = this.moderatedQue.filter((item: { [s: string]: unknown; } | ArrayLike<unknown>) =>
