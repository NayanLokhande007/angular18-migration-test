...
@Component({
  selector: 'app-ad-contributor-profile',
  standalone: true,
  imports: [],
  templateUrl: './ad-contributor-profile.component.html',
  styleUrl: './ad-contributor-profile.component.css'
})
export class AdContributorProfileComponent {
  data: any;
  options: any;
  ...
  constructor(private fb: FormBuilder, private globalservice: GlobalServiceService, private router: Router) { 
    this.getTotalContributedQuestions();
    this.getTotalAcceptedQuestions();
    this.getPendingQuestionsOfContributor();
  ...
  getTotalContributedQuestions() {
    this.globalservice.getTotalContributedQuestions(this.emailid).subscribe(result => {
  ...
  getTotalAcceptedQuestions() {
    this.globalservice.getTotalAcceptedQuestions(this.emailid).subscribe(result => {
      const responsedata = JSON.parse(JSON.stringify(result));
      if (responsedata.status == 200) {
        this.accepted = responsedata.data; 
      }
    });
  }

  getPendingQuestionsOfContributor() {
    this.globalservice.getPendingQuestionsOfContributor(this.emailid).subscribe(result => {
      const responsedata = JSON.parse(JSON.stringify(result));
      if (responsedata.status == 200) {
  ...
  RenderBarChart(chartid: string, charttype: any) {
    ...
    this.globalservice.getMonthWiseAcceptedVaritionsCountofContributor(this.emailid, this.currentYear).subscribe(result => {
      const responsedata = JSON.parse(JSON.stringify(result));
      if (responsedata.status == 200) {
